...
# Imports for the egnn.
import torch.nn as nn
from torch.nn import Module, ModuleList, Linear
from torch_geometric.nn import global_mean_pool, MLP
...
"""
TODO: Remove all the other stuff in this file. The below implementation is the original
EGNN which doesn't exactly work like the one we need.
"""

class EGNNold(nn.Module):
...
class EGNN(Module):
    def __init__(
        ...
        for i in range(len(hidden_channels) - 1):
            in_dim = hidden_channels[i]
            out_dim = hidden_channels[i + 1]

            use_residual = in_dim == out_dim  # Only apply if shapes match
    ...
    def forward(self, x, edge_index, batch, edge_attr=None, pos=None):
        ...
        for layer in self.layers:
            h, coord, _ = layer(h, edge_index, coord, edge_attr=edge_attr)
