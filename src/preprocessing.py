...
def load_eigs_txt(txt_path: Path, num_eigs: int) -> Optional[torch.Tensor]:
    ...
    try:
        ...
        if len(vals) < num_eigs:
            return None
        return torch.tensor(vals, dtype=torch.float32)  # [num_eigs]
    except Exception:
        return None
...
def mesh_to_data(
    ...
    Convert a single OBJ mesh into a torch_geometric Data:
      ...
      - edge_index: [2, E]
      - y: [1, num_eigs]  <-- row vector so PyG batches to [B, num_eigs]
    """
    ...
    if eigs_index is not None:
        ...
        if y is None and strict_targets:
            print(f"[warn] Missing/invalid eigs for {mesh_path.name} (expected eigs/{stem}.txt); skipping.")
            return None

    # --- KEY CHANGE: make y a row vector so batching yields [B, num_eigs] ---
    if y is not None:
        if y.ndim == 1:
            y = y.unsqueeze(0)   # [1, num_eigs]
        elif y.ndim == 2 and y.size(0) != 1:
            # Defensive: ensure it is [1, num_eigs]
            y = y.view(1, -1)
...
def build_mesh_dataset(
    ...
    if eigs_dir is None:
        dataset_root = meshes_dir.parent
    ...
    if save_path is not None:
        ...
        # quick sanity: load then report a small batch's y shape
        try:
            loaded = torch.load(save_path, map_location="cpu", weights_only=False)
            if loaded and hasattr(loaded[0], "y") and loaded[0].y is not None:
                from torch_geometric.loader import DataLoader as _DL
                _ldr = _DL(loaded[:4], batch_size=2, shuffle=False)
                b = next(iter(_ldr))
                print(f"[sanity] Batch y shape: {tuple(b.y.shape)} (expect (B, {loaded[0].y.size(1)}))")
        except Exception as e:
            print(f"[sanity] Skipped batch shape check: {e}")
...
if __name__ == "__main__":
    ...
    if args.data == "All":
        datasets = ["ModelNet10", "SynthNet", "Thingi10k", "Tori"]
    else:
        datasets = [args.data]
    
    for data in datasets:
        meshes_dir = f"../../../data/processed_{data}/meshes"
        eigs_dir   = f"../../../data/processed_{data}/eigs"
        save_path  = f"../data_processed/processed_{data}.pt"
        ...
        if args.test:
            ...
            for i, batch in enumerate(loader):
                ...
                # logits = model(batch.x, batch.edge_index, batch.batch, pos=batch.pos)
